...
| per-parameter signs (A2) | the gate needs `+1` while mu needs `-1`; one multiclass booster has ONE global vector: with `[-1]`, P(zero) is *forbidden from rising with price* — either it violates monotonicity through `(1-pi)` rising, or it deletes the price->occurrence channel entirely (zero splits) and miscalibrates the zero rate against price | V2 num_class demo: both outputs forced non-increasing (max step 0.0, 0.0), the +slope target crushed (RMSE 1.73 vs 0.004); model B: 70–85% sweep violations under three configs, 0 gate price-splits under a fourth |
